                    ...
                    Context ctx, Runtime *runtime)
{
  ...
  IndexPartition cell2vertices_equal_ip = 
      runtime->create_equal_partition(ctx, all_cells_to_vertices_lr.get_index_space(), partition_is);
  LogicalPartition cell2vertices_equal_lp = runtime->get_logical_partition(ctx, all_cells_to_vertices_lr, cell2vertices_equal_ip);
  ...
  init_launcher.region_requirements[0].add_field(FID_CELL_VERTEX_NRANGE);
  ...
  init_launcher.add_region_requirement(
        RegionRequirement(cell2vertices_equal_lp, 0/*projection ID*/,
                          WRITE_DISCARD, EXCLUSIVE, all_cells_to_vertices_lr));
  init_launcher.region_requirements[2].add_field(FID_CELL_TO_VERTEX_ID);
  init_launcher.region_requirements[2].add_field(FID_CELL_TO_VERTEX_PTR);
  ...
  test_launcher.region_requirements[6].add_field(FID_VERTEX_PARTITION_COLOR);
...
//  printf("\n-----------Running init for point %d, rows_per_partition %d, rows %d, rows_partition %d, host %s -----------\n", 
  ...
  const AccessorRWrect cells_vertices_nrange_acc(regions[0], FID_CELL_VERTEX_NRANGE);
  ...
  const AccessorRWint cells_to_vertices_id_acc(regions[2], FID_CELL_TO_VERTEX_ID);
  const AccessorRWpoint cells_to_vertices_ptr_acc(regions[2], FID_CELL_TO_VERTEX_PTR);
...
//  printf("Owned at point %d...\n", point);
  ...
  Rect<1> rect_cell2vertices = runtime->get_index_space_domain(ctx,
                  task->regions[2].region.get_index_space());
  
  int *cells_to_cells_id_ptr = cells_to_cells_id_acc.ptr(rect_cell2cell.lo);
  int *cells_to_vertices_id_ptr = cells_to_vertices_id_acc.ptr(rect_cell2vertices.lo);
  ...
  for (PointInDomainIterator<1> pir(domain_owned); pir(); pir++) {
    ...
    cells_vertices_nrange_acc[*pir] = Rect<1>(cell_id*4, cell_id*4+3);
    ...
    cells_to_vertices_id_ptr[ct*4+0] = cell_id + y_idx;
    cells_to_vertices_id_ptr[ct*4+1] = cells_to_vertices_id_ptr[ct*4+0] + 1;
    cells_to_vertices_id_ptr[ct*4+2] = cell_id + y_idx + (num_rows+1);
    cells_to_vertices_id_ptr[ct*4+3] = cells_to_vertices_id_ptr[ct*4+2] + 1;
...
//			printf("\n");
  ...
  for (PointInRectIterator<1> pir(rect_cell2vertices); pir(); pir++) {
    cells_to_vertices_ptr_acc[*pir] = Point<1>(cells_to_vertices_id_acc[*pir]); 
  }
}
                ...
                Context ctx, Runtime *runtime)
{
  ...
  const AccessorROint vertices_id_ghost_acc(regions[7], FID_VERTEX_ID);
  ...
  const AccessorROint vertices_id_shared_acc(regions[8], FID_VERTEX_ID);
                ...
                Context ctx, Runtime *runtime)
{
  ...
  printf("ghost preimage/image for point %d...\n", point);
  Domain domain_owned = runtime->get_index_space_domain(ctx,
                  task->regions[0].region.get_index_space());
  for (PointInDomainIterator<1> pir(domain_owned); pir(); pir++)
