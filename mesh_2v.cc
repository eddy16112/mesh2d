                    ...
                    Context ctx, Runtime *runtime)
{
  ...
  printf("Running mesh for %d cells, %d rows, %d columns, %d partitions...\n", num_cells, num_rows, num_rows, num_partitions);
  // index space
  ...
  IndexSpace cells_to_cells_index_space = runtime->create_index_space(ctx, Rect<1>(0, num_cells*4-1));
  ...
  for (i = 0; i < num_cells; i++) {
    cells_id_acc[i] = i;
    cells_cells_nrange_acc[i] = Rect<1>(i*4, i*4+3);
    ...
    cells_to_cells_id_acc[i*4+0] = i-1;
    cells_to_cells_id_acc[i*4+1] = i+1;
    cells_to_cells_id_acc[i*4+2] = i-num_rows;
    cells_to_cells_id_acc[i*4+3] = i+num_rows;
    
    // top
    if (i < num_rows) {
     // cells_to_cells_id_acc[i*4+2] = i + num_rows * (num_rows - 1);
      cells_to_cells_id_acc[i*4+2] = cells_to_cells_id_acc[i*4+3];
    }
    
    // bottom
    if (i >= num_rows * (num_rows - 1)) {
    //  cells_to_cells_id_acc[i*4+3] = i - num_rows * (num_rows - 1);
      cells_to_cells_id_acc[i*4+3] = cells_to_cells_id_acc[i*4+2];
    }
    
    // left
    if (i % num_rows == 0) {
     // cells_to_cells_id_acc[i*4+0] = i + num_rows - 1;
      cells_to_cells_id_acc[i*4+0] = cells_to_cells_id_acc[i*4+1];
    }
    
    // right
    if (i % num_rows == num_rows - 1) {
      //cells_to_cells_id_acc[i*4+1] = i - (num_rows - 1);
      cells_to_cells_id_acc[i*4+1] = cells_to_cells_id_acc[i*4+0];
    }
    
    cells_to_cells_ptr_acc[i*4+0] = Point<1>(cells_to_cells_id_acc[i*4+0]);
    cells_to_cells_ptr_acc[i*4+1] = Point<1>(cells_to_cells_id_acc[i*4+1]);
    cells_to_cells_ptr_acc[i*4+2] = Point<1>(cells_to_cells_id_acc[i*4+2]);
    cells_to_cells_ptr_acc[i*4+3] = Point<1>(cells_to_cells_id_acc[i*4+3]);
    ...
    printf("cell %d, color %lld, left %d, right %d, top %d, bottom %d, nrange(%d, %d), cell2cell neighbors: ", cells_id_acc[i], cells_color_acc[i].x, cells_to_cells_id_acc[i*4+0], cells_to_cells_id_acc[i*4+1],
           cells_to_cells_id_acc[i*4+2], cells_to_cells_id_acc[i*4+3], i*4, i*4+3);
    for (j = i*4; j <= i*4+3; j++) {
  ...
  runtime->execute_index_space(ctx, test_launcher);
  
//  LogicalPartition reachable_nrange_lp = runtime->get_logical_partition(ctx, all_cells_to_cells_lr, reachable_nrange_ip);
...
//  test_range_launcher.region_requirements[0].add_field(FID_CELL_NEIGHBOR_ID);
